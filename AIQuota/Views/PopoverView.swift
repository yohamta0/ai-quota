...
struct PopoverView: View {
        ...
        if viewModel.isCodexAuthenticated {
            if let u = viewModel.codexUsage {
                ...
                CircularGaugeView(
                    ...
                    secondaryLabel: "7d",
                    paceLimitPercent: viewModel.codexDailyPace?.ceiling,
        ...
        if viewModel.isClaudeAuthenticated {
            if let u = viewModel.claudeUsage {
                CircularGaugeView(
                    ...
                    secondaryLabel: "7d",
                    paceLimitPercent: viewModel.claudeDailyPace?.ceiling,
                    resetAt: u.resetAt,
    ...
    private func dailyPaceRow(_ pace: DailyPaceBudget) -> some View {
        compactRow(
            ...
            infoHelp: dailyPaceHelpText(pace)
        )
    }

    private func dailyPaceHelpText(_ pace: DailyPaceBudget) -> String {
        "By the end of today you may be at \(Int(pace.ceiling.rounded()))% of the weekly quota; "
            + "you are at \(Int(pace.utilization.rounded()))%. The cap spreads the quota evenly "
            + "across the window and is counted from when the window opened, not from midnight, "
            + "so it holds still all day and steps up once each night."
    }
